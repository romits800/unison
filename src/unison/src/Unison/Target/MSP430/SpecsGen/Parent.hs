...
module Unison.Target.MSP430.SpecsGen.Parent (parent) where
...
parent i
  | i `elem`
      [ADC16mi, ADC16mm, ADC16mr, ADC16ri, ADC16rm, ADC16rr, ADC8mi,
       ...
       LIFETIME_START, LOAD_STACK_GUARD, LOCAL_ESCAPE, MOV16mi, MOV16mm,
       MOV16mr, MOV16ri, MOV16rm, MOV16rm_POST, MOV16rr, MOV8mi, MOV8mm,
       MOV8mr, MOV8ri, MOV8rm, MOV8rm_POST, MOV8rr, MOVZX16rm8,
       MOVZX16rr8, NOP, OR16mi, OR16mm, OR16mr, OR16ri, OR16rm,
       ...
       XOR16mi, XOR16mm, XOR16mr, XOR16ri, XOR16rm, XOR16rm_POST, XOR16rr,
       XOR8mi, XOR8mm, XOR8mr, XOR8ri, XOR8rm, XOR8rm_POST, XOR8rr,
       ZEXT16r]
    = Nothing
  | i `elem` [Store] = Just MOV16mr
  | i `elem` [Load] = Just MOV16rm
  | i `elem` [Move] = Just MOV16rr
