...
-}
module Unison.Target.MSP430 (target) where
...
import qualified Unison.Target.MSP430.SpecsGen as SpecsGen
...
instance Read MSP430Instruction where
  readsPrec _ strOp = [(SpecsGen.readOp strOp, "")]

-- | Register array
registerArray = [RegisterClass GR16, RegisterClass GR8]

-- | Register atoms
registerAtoms ra = (ra, ra)

-- | Register classes
regClasses = [RegisterClass GR16, RegisterClass GR8]

-- | Individual registers of each register class
registers (RegisterClass GR16) = [R0, R1, R2, R3, R4, R5, R6, R7,
                                  R8, R9, R10, R11, R12, R13, R14, R15]
registers (RegisterClass GR8) = [R0, R1, R2, R3, R4, R5, R6, R7]

-- | Map from infinite register class to register usage
infRegClassUsage _ = error ("infRegClassUsage: This should not happen")
...
instance Read MSP430Register where
...
instance Show MSP430Register where
  show r = case M.lookup r regStrings of
             (Just s) -> s
             Nothing -> error $ "unmatched: show MSP430Register"
...
--  | targetInst is `elem` [Br] = BranchInfo Conditional (Just l)
branchInfo (Branch {oBranchIs = is, oBranchUs = [BlockRef l]})
  | targetInst is `elem` [Bi]   = BranchInfo Unconditional (Just l)
branchInfo (Branch {oBranchIs = is})
  | targetInst is `elem` [Br]  = BranchInfo Unconditional Nothing
branchInfo (Branch {oBranchIs = is})
  | targetInst is `elem` [Bm]  = BranchInfo Unconditional Nothing
