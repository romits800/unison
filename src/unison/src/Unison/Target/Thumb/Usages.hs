module Unison.Target.Thumb.Usages (usages) where
...
itineraryUsage i _
  | i `elem` [TBcc, TBfar] =
      [mkUsage V6_Pipe 1 1]
  | i `elem` [TB, TBLXi, TBLXr, TBX, TBX_CALL, TBX_RET] =
      [mkUsage V6_Pipe 1 3]
  | i `elem` [TBL] =
    -- This should be [mkUsage V6_Pipe 1 4] but Unison fails if so
      [mkUsage V6_Pipe 1 1]
  | i `elem` [TLDRBi, TLDRBr, TLDRHi, TLDRHr, TLDRSB, TLDRSH, TLDRi, 
              TLDRpci, TLDRr, TLDRspi, TLDRpci_cpi,
              TSTRBi, TSTRBr, TSTRHi, TSTRHr, TSTRi, 
              TSTRr, TSTRspi, LOAD, STORE] =
      [mkUsage V6_Pipe 1 2]
  | i `elem` [LDMIA, LDMIA_RET, STMDA, STMDB, STMIA, STMIB] =
      [mkUsage V6_Pipe 1 4]
  | i `elem` [MRS, MSR] =
      [mkUsage V6_Pipe 1 4]
--- Here we should add more stuff
  | i `elem` [TPOP_RET, TPOP_r4_7_RET] =  
      [mkUsage V6_Pipe 1 8]
--  | i `elem` [TPOP2_r4_11_RET] =  
--      [mkUsage V6_Pipe 1 12]
--  | i `elem` [TPUSH2_r4_11, TPOP2_r4_11] = 
--      [mkUsage V6_Pipe 1 9]
  | i `elem` [TPUSH, TPUSH_r4_7, TPOP, TPOP_r4_7] = 
      [mkUsage V6_Pipe 1 5]
  | i `elem` [TPOP, TPUSH] =
      [mkUsage V6_Pipe 1 1]
  | otherwise = [mkUsage V6_Pipe 1 1]
