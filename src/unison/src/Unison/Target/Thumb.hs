...
-}
module Unison.Target.Thumb (target) where
...
import Unison.Target.Thumb.Registers
import Unison.Target.Thumb.OperandInfo
import Unison.Target.Thumb.Transforms
import Unison.Target.Thumb.Usages
import Unison.Target.Thumb.ThumbRegisterDecl
import Unison.Target.Thumb.ThumbResourceDecl
import Unison.Target.Thumb.SpecsGen.ThumbRegisterClassDecl
import Unison.Target.Thumb.SpecsGen.ThumbInstructionDecl
import Unison.Target.Thumb.SpecsGen.ThumbItineraryDecl
...
target =
    API.TargetDescription {
      ...
      API.tBranchInfo       = branchInfo,
      API.tPreProcess       = preProcess,  -- TODO(Romy): To check
      API.tPostProcess      = postProcess,  -- TODO(Romy): To check
      API.tTransforms       = const transforms, -- TODO(Romy): To check
      ...
      API.tResources        = resources, -- TODO(Romy): To check
      API.tUsages           = usages,    -- TODO(Romy): To check
      API.tNop              = const nop,
      API.tReadWriteInfo    = const readWriteInfo,
      API.tImplementFrame   = const implementFrame,  -- TODO(Romy): Doesn't look complete (?)
...
branchInfo _ (Branch {oBranchIs = is, oBranchUs = [BlockRef l, _, _]})
  | targetInst is `elem` [T2Bcc, TBcc] =
    BranchInfo Conditional (Just l)

branchInfo _ (Branch {oBranchIs = is, oBranchUs = [_, BlockRef l]})
...
branchInfo _ (Branch {oBranchIs = is})
  | (targetInst is `elem`
...
branchInfo _ o = error ("unmatched pattern: branchInfo " ++ show (mkSingleOperation (-1) (Natural o)))
...
stackDirection = API.StackGrowsUp

-- | Target dependent pre-processing functions

preProcess to = [
                 mapToTargetMachineInstruction instantiateMEMCPY,
...
altRetConstraints (
  ...
  code) constraints =
  let 
    alt = XorExpr (ActiveExpr (oId op)) (ActiveExpr (oId or))
